...
var gCommScope = {};
Services.scriptloader.loadSubScript('chrome://screencastify/content/resources/scripts/comm/Comm.js', gCommScope);
	...
	ready: function(aContentWindow) {
		...
		switch (pageLoader.matches(contentWindow.location.href, contentWindow.location)) {
			case MATCH_APP:
					...
					console.error('ok matched app setting up gWinComm');
					gCommScope.gWinComm = gWinComm = new gCommScope.Comm.server.content(contentWindow); // cross-file-link884757009
...
function init() {
	gCommScope.gBsComm = gBsComm = new gCommScope.Comm.client.framescript('Screencastify@jetpack');
	...
	callInBootstrap('fetchCore', null, function(aArg, aComm) {
		...
		try {
			...
			console.error('failed to register about:', ignore);
		} // its non-e10s so it will throw saying already registered
		console.error('gAppAboutFactory:', gAppAboutFactory);
